...
`sd`を使うと、AtCoderの問題ページからサンプル入出力を`test/`へ保存できます。
...
問題フォルダ内では引数なし、コンテストフォルダ内では問題記号だけでも実行できます。

```sh
cd contests/atcoder/abc/abc468/a
sd

cd contests/atcoder/abc/abc468
sd a
```

問題フォルダがまだない場合は、テンプレートから自動作成します。既存の空テスト雛形はサンプルで置き換えますが、内容が入っているテストは上書きしません。

VS Codeでは対象の`main.cpp`を開き、「タスクの実行」から「サンプル取得」を選べます。取得後は`st`でまとめて確認します。
